...
function createTooltipDOM(tooltipDOMSpec = [
    ...
    const { dom, contentDOM } = pm.DOMSerializer.renderSpec(window.document, tooltipDOMSpec);
    if (arrow && !dom.querySelector('.bangle-tooltip-arrow')) {
        const arrowElement = pm.DOMSerializer.renderSpec(window.document, [
            'div',
            {
    ...
    class TooltipPlacementView {
        ...
        _createPopperInstance(view) {
            ...
            const defaultModifiers = [
                ...
                core$1.popperOffsets,
                showTooltipArrow ? core$1.arrow : undefined,
                showTooltipArrow
                    ? {
                        name: 'arrow',
                        options: {
            ...
            this.popperInstance = core$1.createPopper(getReferenceElement(view, this._tooltip, this._scrollContainerDOM), this._tooltip, {
                placement,
...
function validateState(state) {
    if (typeof state.show !== 'boolean') {
        utils.bangleWarn(`Tooltip must be controlled by a plugin having a boolean field "show" in its state, but received the state=`, state);
...
let log = () => { };
function selectionTooltip({ key = new pm.PluginKey('selectionTooltipPlugin'), calculateType = (state, _prevType) => {
...
function getSelectionReferenceElement(view) {
    return {
        getBoundingClientRect: () => {
            ...
            const pos = selection instanceof pm.NodeSelection ? from : head;
            ...
            if (selection instanceof pm.NodeSelection) {
...
function triggerInputRule(schema, markName, trigger) {
    ...
    const startRule = new pm.InputRule(regexStart, (editorState, match) => {
        /**
        ...
        if (trigger.length > 1) {
            const textSelection = pm.TextSelection.create(tr.doc, selection.from, selection.from - trigger.length + 1);
...
function pluginsFactory({ key = new pm.PluginKey('suggest_tooltip'), markName, trigger, tooltipRenderOpts, keybindings = defaultKeys, onEnter = (state, dispatch, view) => {
    ...
    return ({ schema }) => {
        ...
        return [
            ...
            plugins$2({
                ...
                renderOpts: {
                    ...
                    getReferenceElement: referenceElement((state) => {
                        ...
                        utils.assertNotUndefined(markType, `markType ${markName} not found`);
                        const { selection } = state;
                        return utils.findFirstMarkPosition(markType, state.doc, selection.from - 1, selection.to);
                    }),
                },
            }),
            ...
            keybindings &&
                pm.keymap(utils.createObject([
                    [keybindings.select, utils.filter(isActiveCheck, onEnter)],
                    [keybindings.up, utils.filter(isActiveCheck, onArrowUp)],
                    [keybindings.down, utils.filter(isActiveCheck, onArrowDown)],
                    [keybindings.left, utils.filter(isActiveCheck, onArrowLeft)],
                    [keybindings.right, utils.filter(isActiveCheck, onArrowRight)],
                    [keybindings.hide, utils.filter(isActiveCheck, onEscape)],
                ])),
        ];
    };
}
...
function replaceSuggestMarkWith(key, maybeNode) {
    return (state, dispatch, view) => {
        ...
        utils.assertNotUndefined(markType, `markType ${markName} not found`);
        const { selection } = state;
        const queryMark = utils.findFirstMarkPosition(markType, state.doc, selection.from - 1, selection.to);
        ...
        const getTr = () => {
            ...
            let tr = state.tr
                ...
                .replaceWith(start, end, pm.Fragment.empty);
            if (!maybeNode) {
                return tr;
            }
            const isInputFragment = maybeNode instanceof pm.Fragment;
            let node;
            try {
                node =
                    maybeNode instanceof pm.Node || isInputFragment
                        ? maybeNode
                        : typeof maybeNode === 'string'
                            ? state.schema.text(maybeNode)
                            : pm.Node.fromJSON(state.schema, maybeNode);
            }
            catch (e) {
                console.error(e);
                return tr;
            ...
            if (node.isText) {
                ...
                tr = utils.safeInsert(node)(tr);
            }
            else if (node.isInline || isInputFragment) {
                const fragment = isInputFragment
                    ? node
                    : pm.Fragment.fromArray([node, state.schema.text(' ')]);
                ...
                tr = tr.setSelection(pm.Selection.near(tr.doc.resolve(start + fragment.size)));
                return tr;
            }
            return tr;
        };
...
exports.createTooltipDOM = createTooltipDOM;
exports.selectionTooltip = selectionTooltip$1;
exports.suggestTooltip = suggestTooltip;
exports.tooltipPlacement = tooltipPlacement$1;
